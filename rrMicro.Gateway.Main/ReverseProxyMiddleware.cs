...
namespace rrMicro.Gateway.Main
{
    public class ReverseProxyMiddleware
    {
        ...
        private Uri BuildTargetUri(HttpRequest request)
        {
            ...
            //var destination = _routes.FirstOrDefault(r => r.Endpoint.Equals(basePath)).Destination;
            var destination = _routes.FirstOrDefault(r => path.StartsWith(r.Endpoint)).Destination;
            if (destination != null)
            {
                var remainingPath = path.Split(destination.Path);
                var res =  new Uri(destination.Path + (remainingPath != null && remainingPath.Count() >= 2 ? remainingPath[1] : ""));
                return res;
            }
